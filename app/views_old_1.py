...
def generate_frames():
    ...
    while True:
        ...
        img_output = img.copy()
        detector.findHands(img)  # still draws hand landmarks on frame

        # Use full frame for prediction (matches Teachable Machine training)
        img_resized = cv2.resize(img, (224, 224))
        prediction, index = classifier.getPrediction(img_resized)
        ...
        if index < len(labels):
            ...
            if detected_label == last_label:
                ...
                # Progress arc (top center of frame)
